...
def gb2gtf( source='gb2gtf',allowedTypes=set(['gene','CDS','tRNA','tmRNA','rRNA','ncRNA']) ):
  ...
  for gb in SeqIO.parse( handle,'gb' ):
    ...
    for f in gb.features:
      ...
      comments=[]
      sys.stderr.write('\t'.join(f.qualifiers.keys())+'\n')
      keys=f.qualifiers.keys()

      ty=f.type
      if f.type=='CDS': ty='exon';tid+=1; comments.append('transcript_id' +' "'+str(tid)+'"')
      else: continue

      for key in keys:
        sys.stderr.write(key+':::'+ f.qualifiers[key][0]+';;;')
        if key=='gene':
          comments.append('gene_id'+' "'+str(tid)+'"')
          comments.append('gene_name'+' "'+f.qualifiers[key][0]+'"')
        elif key=='label' and 'gene' not in keys:
          comments.append('gene_id'+' "'+str(tid)+'"')
          comments.append('gene_name'+' "'+f.qualifiers[key][0]+'"')
        else:
          pass
          #comments.append(key.replace(' ', '_')+' "'+f.qualifiers[key][0]+'"')
      
      sys.stderr.write('\n')
      #print f.type, 'TYPE'
      # if 'gene' in f.qualifiers:
        # comments.append(' gene_id "%s"' % f.qualifiers['gene'][0])
      # if 'protein_id' in f.qualifiers:
        # comments.append('protein_id "%s"' % f.qualifiers['protein_id'][0])
      
      #add external IDs  
      # if 'db_xref' in f.qualifiers:
        # for extData in f.qualifiers['db_xref']:
          # comments.append('db_xref "%s"' % extData)
      ...
      gtf = '%s\t%s\t%s\t%s\t%s\t.\t%s\t.\t%s' % ( acc,source,ty,f.location.start.position+1,f.location.end.position,strand,' ; '.join(comments) ) #f.frame,
      print gtf
