...
servers=['bsidna3','bsidna4','bsidna5','bsidna6','bsidna7','bsidna8','bsidna9','bsidna10',\
...
def trim():
	...
	for (key, fqfiles) in seeds.items():
		...
		#ff = open(wd+'/soap_config/'+key+'_soap.config', 'w')
			...
			f3.write(serverTag + ' '+dirscr+'blast_trim.py '+fid+'.dup '+fid+'.tab '+fid+'.fq '+ base+'_'+key+ ' '+str(i)+' &\n') #adaptor trimming
			f4.write(serverTag + ' '+dirscr+'trim_quality.py '+fid+'.fq '+wd+'/fastq/' + key+'_'+str(i)+'_sequence.txt 33 '+fastqfile+' &\n') #quality trimming
			if job%nservers==0: 
				f.write('wait\n')
				f1.write('wait\n')
				f2.write('wait\n')
...
def mira4():
	...
	for (key, vdict) in seeds.items():
		...
		ff.write('parameters = -GE:not=8 -DI:trt=/home/BSI/306307/ -OUTPUT:rtd=yes\n')
